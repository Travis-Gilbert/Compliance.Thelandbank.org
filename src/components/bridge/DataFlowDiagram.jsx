...
function StageCard({ stage, index }) {
  return (
    <div className="relative flex flex-col items-center text-center">
      ...
      {/* Card */}
      <div className="w-full px-3 py-3 rounded-lg border border-accent/20 bg-accent/5 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md hover:bg-accent/10">
        <div className="w-9 h-9 rounded-lg bg-accent text-white flex items-center justify-center mx-auto mb-2 shadow-sm">
          <AppIcon icon={stage.icon} size={18} />
        </div>
        <h4 className="text-xs font-bold text-text leading-tight mb-0.5">{stage.label}</h4>
        <p className="text-[10px] text-muted leading-snug">{stage.detail}</p>
      </div>
    </div>
  );
}
...
function FlowArrow({ connection }) {
  return (
    <div className="flex flex-col items-center justify-center px-1 self-center flex-shrink-0">
      <span className="text-[9px] font-semibold text-muted mb-0.5 whitespace-nowrap">{connection.label}</span>
      <div className="flex items-center">
        <div className="w-6 h-[1.5px] bg-accent/30" />
  ...
  return (
    <div className="w-full">
      ...
      <div className="mt-4 flex items-center gap-2.5 px-4 py-2.5 rounded-lg bg-accent/5 border border-accent/15 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md">
        ...
        <span className="text-[10px] text-muted">— compliance notices cycle back to buyers via unique secure links</span>
      </div>
    </div>
  );
}
